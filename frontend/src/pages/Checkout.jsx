...
export default function Checkout() {
  ...
  const onPaymentProofChange = (e) => {
    ...
    if (!['image/jpeg', 'image/png', 'image/webp'].includes(file.type) || file.size > 5 * 1024 * 1024) {
      setErrors((current) => ({ ...current, paymentProof: 'Choose a JPG, PNG, or WebP image under 5 MB' }));
      e.target.value = '';
      setPaymentProof(null);
      return;
    }
    ...
    reader.onload = () => {
      setPaymentProof({ name: file.name, data: reader.result });
      setErrors((current) => ({ ...current, paymentProof: '' }));
    };
  ...
  const validate = () => {
    ...
    if (form.payment !== 'cod' && !paymentProof) next.paymentProof = 'Upload your payment screenshot';
              ...
              {form.payment === 'esewa' && (
                <div className="payment-qr">
                  ...
                  <label className="payment-proof-upload">
                    Payment screenshot *
                    <input type="file" accept="image/jpeg,image/png,image/webp" onChange={onPaymentProofChange} />
                    <small>Upload the screenshot after completing your eSewa payment (max 5 MB).</small>
                    {paymentProof && <small className="upload-success">Selected: {paymentProof.name}</small>}
              ...
              {form.payment === 'khalti' && (
                <div className="payment-qr">
                  ...
                  <label className="payment-proof-upload">
                    Payment screenshot *
                    <input type="file" accept="image/jpeg,image/png,image/webp" onChange={onPaymentProofChange} />
                    <small>Upload the screenshot after completing your Khalti payment (max 5 MB).</small>
                    {paymentProof && <small className="upload-success">Selected: {paymentProof.name}</small>}
                    {errors.paymentProof && <span className="field-error">{errors.paymentProof}</span>}
                  </label>
                </div>
